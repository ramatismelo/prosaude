...
public class RelaAssoPref {
	public static void configure(Database database) {
		TableRegistry tableRegistry = database.addTableRegistry("relaassopref");
	    ...
	    rmGridRegistry.addLoadRmGridEventListener(new LoadRmGridEventListener() {
			...
			public void onLoadRmGrid(LoadRmGridEvent event) {
				...
				controlForm.setTitle("Estatística de associados por prefixo:");
				controlForm.setWidth(600d);
				controlForm.setHeight(300d);
				
				controlForm.addNewLine();
				controlForm.addSection("associado", "Informe o prefixo (em branco para todos):", SectionState.MAXIMIZED);
				{
					controlForm.addNewLine();
		...
		CommandRegistry commandRegistry = database.addCommandRegistry("relaassopref");
		commandRegistry.addCommandEventListener(new CommandEventListener() {
			@Override
			public void onCommandExecute(CommandEvent event) {
				RmGrid rmGrid = database.loadRmGridByName("relaassopref");
					...
					buttonBase2.addRmFormButtonClickEventListener(new RmFormButtonClickEventListener() {
						...
						public void onRmFormButtonClick(RmFormButtonClickEvent event) {
							if (rmGrid.getForm().getTable().validate()) {
								...
								String urlLink = "http://" + tblParametros.getString("reportServer") + "/php-app/relaassopref.php";
								String urlPdf = null;
