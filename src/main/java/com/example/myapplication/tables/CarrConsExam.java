...
public class CarrConsExam {
	public static void configure(Database database) {
		TableRegistry tableRegistry = database.addTableRegistry("carrconsexam");
		tableRegistry.addLoadTableEventListener(new LoadTableEventListener() {
			...
			public void onLoadTable(LoadTableEvent event) {
				...
				tblTabela.setTableName("carrconsexam");
				...
				tblTabela.addField("desccodiespe", FieldType.VARCHAR, 250);
				...
				tblTabela.setOrder("carrconsexam.codiforn, especialidades.tipoproc, especialidades.grupo, especialidades.descricao");
				
				tblTabela.addJoin("especialidades", "codiespe", "codiespe");
				tblTabela.fieldByName("tipoproc").setExpression("especialidades.tipoproc");
				...
				tblTabela.fieldByName("grupo").setExpression("especialidades.grupo");
				...
				existenceCheck.addAfterExistenceCheckEventListener(new AfterExistenceCheckEventListener() {
					...
					public void onAfterExistenceCheck(AfterExistenceCheckEvent event) {
						...
						event.getSourceTable().setValue("uidfornecedor", event.getTargetTable().getString("uidfornecedor"));
						event.getSourceTable().setValue("codiforn", event.getTargetTable().getInteger("codiforn"));
						event.getSourceTable().setValue("codimedi", resultSet.getInt("codimedi"));
				...
				*/
				...
				tblTabela.addDirectProcessingEventListener(new DirectProcessingEventListener() {
					...
					public void onDirectProcessing(DirectProcessingEvent event) {
						...
						event.getTable().getMasterTable().setInteger("numeitens", event.getTable().getMasterTable().getInteger("numeitens")+1);
	    ...
	    RmGridRegistry rmGridRegistry = database.addRmGridRegistry("carrconsexam");
	    rmGridRegistry.addLoadRmGridEventListener(new LoadRmGridEventListener() {
			@Override
			public void onLoadRmGrid(LoadRmGridEvent event) {
				Table table = event.getTable();
				if (table==null) {
					table = database.loadTableByName("carrconsexam");
				}
				
				RmGrid rmGrid = event.getRmGrid();
				rmGrid.setTable(table);
				rmGrid.setHeight("600px");
				rmGrid.addField("desccodiespe", "Exame/Procedimento", 550d);
				rmGrid.addField("vlrvenda", "Valor", 100d);
				rmGrid.addField("codiforn", "Cód.Fornecedor", 100d);
				rmGrid.addField("tipoproc", "Procedimento", 100d);
				...
				rmGrid.setShowDeleteColumn(true);
		...
		CommandRegistry commandRegistry = database.addCommandRegistry("carrconsexam");
		commandRegistry.addCommandEventListener(new CommandEventListener() {
			@Override
			public void onCommandExecute(CommandEvent event) {
				RmGrid rmGrid = database.loadRmGridByName("carrconsexam");
