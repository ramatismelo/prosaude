...
public class SequCaixC1 {
	public static void configure(Database database) {
		TableRegistry tableRegistry = database.addTableRegistry("sequcaixc1");
		tableRegistry.addLoadTableEventListener(new LoadTableEventListener() {
			...
			public void onLoadTable(LoadTableEvent event) {
				...
				tblTabela.setTableName("sequcaixc1");
		...
		CommandRegistry commandRegistry = database.addCommandRegistry("sequcaixc1");
		commandRegistry.addCommandEventListener(new CommandEventListener() {
			@Override
			public void onCommandExecute(CommandEvent event) {
				RmGrid rmGrid = database.loadRmGridByName("sequcaixc1");
				rmGrid.updateContent();

				ApplicationUI ui = (ApplicationUI) UI.getCurrent();
				ui.getSystemView().addTab(rmGrid, "Sequencias do caixa c1", true);
			}
		});
	}
}
