...
public class Especialidades {
	public static void configure(Database database) {
		TableRegistry tableRegistry = database.addTableRegistry("especialidades");
...
//				table.fieldByName("descagendamento").setValueOfInternalSearch("agendamento");
				...
				ComboBox comboBox = table.fieldByName("uidfornecedor").addComboBox("fornecedores", "descricao", "descricao", "uid");
				comboBox.setAdditionalFieldsToLoad("codiforn");
				comboBox.addComboBoxValueChangeEventListener(new ComboBoxValueChangeEventListener() {
					@Override
					public void onComboBoxValueChange(ComboBoxValueChangeEvent event) {
						event.getSourceTable().setValue("codiforn", event.getSimpleRecord().getInteger("codiforn"));
	    ...
	    RmGridRegistry rmGridRegistry = database.addRmGridRegistry("especialidades");
	    rmGridRegistry.addLoadRmGridEventListener(new LoadRmGridEventListener() {
			...
			public void onLoadRmGrid(LoadRmGridEvent event) {
				...
				rmGrid.addField("desccodiforn", "Fornecedor", 150d);
				rmGrid.addField("desctipoproc", "Procedimento", 200d);
				rmGrid.addField("vlrcusto", "Valor Custo", 100d);
				...
				controlForm.addSection("especilidades", "Controle de especialidades.", SectionState.MAXIMIZED);
				{
					...
					controlForm.addField("tipoproc", "Procedimento", 200d);
					
					controlForm.addNewLine();
					controlForm.addField("uidfornecedor", "Fornecedor", 100d, 1);
		...
		CommandRegistry commandRegistry = database.addCommandRegistry("especialidades");
		commandRegistry.addCommandEventListener(new CommandEventListener() {
			@Override
			public void onCommandExecute(CommandEvent event) {
				RmGrid rmGrid = database.loadRmGridByName("especialidades");
